<template>
  <div :class="numberinputCls">
    <div class="blue-numberinput-show" :class="{'focusing':focusing}">
      <input
        type="text"
        :placeholder="placeholder"
        :disabled="disabled"
        class="blue-numberinput-input blue-input"
        v-model="editValue"
        @input="input"
        @focus="focusing=true"
        @blur="blur"
      >
      <div class="blue-numberinput-operate" v-if="useOperate">
        <span @click="minus">
...
const prefix = "blue-numberinput";

export default {
  name: "blueNumberInput",
  props: {
    value: [Number, String],
    min: Number,
    max: Number,
  ...
  methods: {
    ...
    input(event) {
      ...
      if (
        utils.isNumber(this.value) &&
        Math.abs(value - this.value) <= 1 &&
        this.precision
      ) {
        return;
      }
    ...
    setvalue(value, trigger) {
      ...
      if (this.precision && utils.isNumber(value)) {
        value =
          Math.floor(utils.mul(value || 0, Math.pow(10, this.precision))) /
