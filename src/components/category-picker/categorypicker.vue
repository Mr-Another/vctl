<template>
  <div :class="categoryCls" :disabled="disabled">
    <div class="blue-categorypicker-show">
      <div v-if="multiple&&objects.length" class="blue-categorypicker-multiple-tags">
        <span v-for="(obj, index) of objects" :key="index+''+obj.key">
          <span>{{getShow(obj)}}</span>
          <i class="icon-close-min" @click.stop="remove(obj)" v-if="!disabled"></i>
        </span>
      </div>
      <div v-else-if="!multiple&&object" class="blue-categorypicker-value-single">
        <span>{{getShow(object)}}</span>
        <i class="icon-close" v-if="object&&!disabled" @mousedown="clear"></i>
      </div>
      <div v-else class="blue-categorypicker-placeholder">{{showPlaceholder}}</div>
    </div>
    <div :class="groupCls">
      <Tabs
        :datas="tabs"
        v-model="tab"
        class="blue-categorypicker-tabs"
        keyName="key"
        titleName="title"
        @change="focusTab"
      ></Tabs>
      <div class="blue-categorypicker-ul" :class="{'blue-categorypicker-single-picker': !multiple}">
        <div
          v-for="data of list"
          :key="data.key"
          class="blue-categorypicker-item"
          :class="{'blue-categorypicker-item-selected': object && data.key == object.key}"
        >
          <i class="icon-loading" v-if="data.status.loading"></i>
          <Checkbox
            v-else-if="data.status.checkable&&multiple"
            :checked="isChecked(data)"
            @click.native="change(data, $event)"
          ></Checkbox>
          <span class="blue-categorypicker-item-title" @click="openNew(data, $event)">
            {{data.title}}
            <span
              v-if="showChildCount && data.children.length"
...
const prefix = "blue-categorypicker";
const topMenu = "-------";

export default {
  name: "blueCategoryPicker",
  mixins: [Locale],
  components: { Checkbox },
  props: {
    option: Object,
  ...
  methods: {
    init() {
      this.$nextTick(() => {
        let el = (this.el = this.$el.querySelector(
          ".blue-categorypicker-show"
        ));
        this.content = this.$el.querySelector(".blue-categorypicker-group");
        let that = this;
    ...
    dispose() {
      if (this.multiple) {
        return this.objects
          ...
          .map(item =>
            this.type == "key" ? item : this.getDisposeValue(item)
          );
      } else if (this.object) {
        return this.type == "key"
          ? this.object.key
          : this.getDisposeValue(this.object.value);
      }
      return null;
    },
    ...
    initCategoryDatas() {
      ...
      if (
        utils.isFunction(this.param.getTotalDatas) ||
        utils.isFunction(this.param.getDatas)
      ) {
        datas = [];
      ...
      for (let data of list) {
        let obj = {
          ...
          status: {
            ...
            opened: false,
            selected: false,
            selectable: this.param.selectable
              ? this.param.selectable(data, level)
              : data.selectable !== false,
            checkable: this.param.checkable
              ? this.param.checkable(data, level)
              : data.checkable !== false
          }
        };
        let children = data[this.param.childrenName] || [];
        obj.children = this.initTreeModeData(
          children,
          obj.key,
          isWait,
          level + 1
    ...
    openNew(data, event) {
      ...
      if (data.children && data.children.length) {
        ...
        this.param.getDatas.call(
          ...
          data.value,
          result => {
            data.children = this.initTreeModeData(
              result,
              data.key,
              true,
              data.status.level + 1
    ...
    change(data, event) {
      ...
      if (this.multiple) {
        if (
          this.objects.length >= this.limit &&
          !this.param.objects.some(item => item.key === data.key)
        ) {
          Message.error(
  ...
  computed: {
    ...
    param() {
      if (this.config) {
        return utils.extend(
          {},
          config.getOption("categoryPicker.default"),
          config.getOption(`categoryPicker.configs.${this.config}`),
          this.option
        );
      } else {
        return utils.extend(
