<template>
  <div class="blue-checkbox" :disabled="disabled" :class="{'blue-checkbox-disabled': disabled}">
    <template v-if="!isSingle">
      <label
        v-for="option of arr"
        @click="setvalue(option)"
        :key="option[key]"
        :class="{'blue-checkbox-checked': isInclude(option)}"
      >
        <span :checked="isInclude(option)" :disabled="disabled" class="blue-checkbox-native"></span>
        <span class="blue-checkbox-text" v-if="!$scopedSlots.item">{{option[title]}}</span>
    ...
    <label
      v-else
      @click="setvalue()"
      :class="{'blue-checkbox-checked': isChecked, 'blue-checkbox-indeterminate': !isChecked&&indeterminate}"
    >
      <span
        :checked="isChecked"
        :indeterminate="!isChecked&&indeterminate"
        :disabled="disabled"
        class="blue-checkbox-native"
      ></span>
      <span v-if="$slots.default" class="blue-checkbox-text">
        <slot></slot>
      </span>
    </label>
  </div>
...
export default {
  ...
  computed: {
    checkList() {
      ...
      if (
        (!utils.isNull(this.value) || !this.isSingle) &&
        !utils.isArray(checkStatus)
      ) {
        console.warn(
          `[BlueUI WARNING] Checkbox Component: It's not allowed to use v-model with non-array value.`
        );
      }
