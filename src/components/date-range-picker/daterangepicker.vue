<template>
  <div :class="dateCls">
    ...
    <div :class="datePickerCls" class="blue-date-picker">
      <div class="blue-date-container blue-date-range-container" v-if="isShow">
...
export default {
  ...
  data() {
    let format =
      this.format || config.getOption("datepicker.format")[this.type];
  ...
  methods: {
    ...
    updateView(value, rangeType) {
      this.nowView[rangeType] = mm(value);
      if (this.nowView.start.time() >= this.nowView.end.time()) {
        if (rangeType == "end") {
          this.nowView.start = mm(value).add(-1, mm.MONTH);
        } else {
          this.nowView.end = mm(value).add(1, mm.MONTH);
        }
      }
    ...
    initNowView() {
      ...
      this.nowView = {
        start,
        end: mm(start).add(endRange, mm.MONTH)
      };
  ...
  computed: {
    ...
    show() {
      ...
      return `${this.value.start || this.t("h.datepicker.start")} - ${this.value
        .end || this.t("h.datepicker.end")}`;
    },
    shortcuts() {
      let shortcuts = [];
