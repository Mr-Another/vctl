<template>
  <div class="blue-category-modal">
    <header class="relative" v-if="params.title">{{params.title}}</header>
    <div>
      <div class="blue-panel-bar">
        <div class="blue-category-modal-multiple-tags" v-if="param.multiple">
          <span v-for="tag of param.objects" :key="tag.key"><span>{{tag.title}}</span><i class="icon-close-min"
              @click.stop="remove(tag)"></i></span>
        </div>
        <div v-else class="blue-category-modal-single-tag">
          <span v-if="param.object">{{param.object.title}}</span>
        </div>
        <Search v-if="param.filterable" v-model="searchText" trigger="input" class="blue-panel-right"></Search>
      </div>
      ...
      <div class="blue-panel-body">
        <Row :space="10">
          <template v-if="searchText==''">
            ...
            <div class="text-ellipsis blue-category-item" @click="openNew(data)">
              ...
              <Checkbox v-else-if="data.status.checkable" :checked="isChecked(data)" @click.native="change(data, $event)"></Checkbox><i
                class="blue-split"></i>{{data.title}} <span v-if="data.children.length">({{data.children.length}})</span>
            </div>
            </Cell>
          </template>
          <Cell v-else :width="8" v-for="data of searchlist" :key="data">
          <div class="text-ellipsis blue-category-item" @click.stop="change(data)">
            <Checkbox v-if="data.status.checkable" :checked="isChecked(data)" @click.native="change(data, $event)"></Checkbox><i
              class="blue-split"></i>{{data.title}}
          </div>
          </Cell>
        </Row>
      </div>
    ...
    <footer>
      <button class="blue-btn blue-btn-primary" @click="confirm">{{'h.common.confirm' | hlang}}</button>
      <button class="blue-btn" @click="close">{{'h.common.cancel' | hlang}}</button>
    </footer>
  </div>
</template>
<script>
