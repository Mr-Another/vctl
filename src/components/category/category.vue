<template>
  <div :class="categoryCls" :disabled="disabled">
    <div class="blue-category-show" @click="openPicker">
      <div v-if="multiple&&objects.length" class="blue-category-multiple-tags">
        <span v-for="obj of objects" :key="obj.key">
          <span>{{obj.title}}</span><i class="icon-close-min" @click.stop="remove(obj)" v-if="!disabled"></i>
        </span>
      </div>
      <div v-else-if="!multiple&&object" class="blue-category-value-single">
        <span>{{object.title}}</span>
        <i class="icon-close" v-if="object.title!=null&&!disabled" @mousedown="clear"></i>
      </div>
      <div v-else class="blue-category-placeholder">{{showPlaceholder}}</div>
      <!-- <i class="icon-down"></i> -->
    </div>
  </div>
</template>
