<template>
  <div :class="dateCls">
    <div v-if="noBorder" class="blue-datetime-show text-hover">{{showValue||placeholder}}</div>
    ...
    <div :class="datePickerCls" class="blue-date-picker">
      <div class="blue-date-container blue-date-full-range-container">
        ...
        <div v-if="view == 'customize'" class="blue-date-self-defined">
          <DatePicker
      ...
      <div class="blue-date-footer">
        <button
          class="blue-btn blue-btn-text blue-btn-s"
          @click="clear"
        >{{'h.common.clear' | hlang}}</button>
        <button
  ...
  methods: {
    ...
    parseSingle(value, range) {
      if (utils.isObject(value) && value[this.paramName[range]]) {
        try {
          let nowValue = mm(value[this.paramName[range]]);
          if (range == "end") {
    ...
    initNowView() {
      ...
      if (this.nowDate.start) {
        start = mm(this.nowDate.start);
      }
      let end = mm();
      if (this.nowDate.end) {
        end = mm(this.nowDate.end);
      }
      this.nowView = {
        start,
        end
    ...
    setvalue(string, isEnd = false) {
      ...
      if (this.view == "week") {
        ...
        value = {
          start: start.format(),
          end: start.add(1, mm.YEAR).format()
        };
      } else if (this.view == "month") {
        value = {
          start: start.format(),
          end: start.add(1, mm.MONTH).format()
        };
      } else if (this.view == "quarter") {
        value = {
          start: start.format(),
          end: start.add(3, mm.MONTH).format()
        };
      } else if (this.view == "date") {
        value = {
          start: start.format(),
          end: start.add(1, mm.DAY).format()
        };
      }
      if (!value.start) {
        value.start = null;
  ...
  computed: {
    views() {
      ...
      for (let l of this.layout) {
        if (!this.allviews[l]) {
          console.warn(
            `[BlueUI WARNING] DateFullRangePicker Component: Props ${l} for layout don't exsits.`
          );
        } else {
          v[l] = this.allviews[l];
        }
