<template>
	<div :class="tableCls">
		<div class="blue-table-header" :style="{'padding-right': (scrollWidth+'px')}">
			<table :style="{'margin-left': (-scrollLeft+'px')}">
				...
				<template v-if="ths">
					<tr v-for="(thdata, thindex) of ths" :key="thindex+update.columns">
						...
						<TableTh
							v-for="(thdata, index) of thdata"
							:key="index+update.columns"
							v-bind="thdata"
			...
			<div class="blue-table-fixed-cover" :style="{'width': (scrollWidth+'px')}"></div>
		</div>

		<div class="blue-table-container">
			<div class="blue-table-content-empty" v-if="labelData.length == 0">
				<slot name="empty"></slot>
				<div v-if="!$slots.empty">{{'h.table.empty' | hlang}}</div>
			</div>
			<div class="blue-table-body" v-show="labelData.length" :style="bodyStyle">
				<table>
					<colgroup>
						<col v-if="checkbox" width="40" />
						<col v-for="(c, index) of computeColumns" :width="getWidth(c)" :key="index+update.columns" />
					</colgroup>
					<tbody class="blue-table-tbody">
						<template v-for="(d, index) of labelData">
							<TableTr
								@click="triggerTrClicked"
								@dblclick="triggerTrDblclicked"
								:datas="d"
								:key="index+update.datas"
								:index="index"
								:trIndex="index"
								:class="getTrCls(d, index)"
							>
								<td v-if="checkbox" class="blue-table-td-checkbox">
									<Checkbox v-if="fixedColumnLeft.length==0" v-model="checks" :value="d"></Checkbox>
								</td>
								<slot :data="d" :index="index" v-if="isTemplateMode"></slot>
							</TableTr>
							<tr
								:key="index+update.datas+'expand'"
								class="blue-table-expand-tr"
								v-if="$scopedSlots.expand && d._expand"
							>
								<td class="blue-table-expand-cell" :colspan="totalCol">
			...
			<div v-if="fixedColumnRight.length" class="blue-table-fixed-right" :style="fixedRightBodyStyle">
				<table :style="{'margin-top': (-scrollTop+'px'), width: (tableWidth + 'px')}">
					<colgroup>
						<col v-for="(c, index) of computeColumns" :width="getWidth(c)" :key="index+update.columns" />
					</colgroup>
			...
			<table :style="{width: leftWidth + 'px'}">
				...
				<tr>
					<th v-if="checkbox" class="blue-table-th-checkbox">
						<Checkbox
							:indeterminate="checks.length>0&&checks.length<labelData.length"
							:checked="labelData.length > 0 && checks.length == labelData.length"
							@click.native="checkAll"
						></Checkbox>
					</th>
					<TableTh
						v-for="(thdata, index) of fixedColumnLeft"
						:key="index+update.columns"
						v-bind="thdata"
						:sortStatus="sortStatus"
					></TableTh>
		...
		<div
			v-if="fixedColumnRight.length"
			:style="{'margin-right': (scrollWidth+'px')}"
			class="blue-table-fixed-header-right"
		>
			<table :style="{width: rightWidth + 'px'}">
				...
				<tr>
					<TableTh
						v-for="(thdata, index) of fixedColumnRight"
						:key="index+update.columns"
						v-bind="thdata"
						:sortStatus="sortStatus"
					></TableTh>
		...
		<div class="blue-table-items" v-if="!isTemplateMode">
			<slot></slot>
		</div>
...
export default {
	...
	mounted() {
		...
		this.initColumns()
		this.$nextTick(() => {
			let body = this.$el.querySelector('.blue-table-body')
			if (body) {
				...
				let fixedright = this.$el.querySelector(
					'.blue-table-fixed-right'
				)
				let fixedleft = this.$el.querySelector('.blue-table-fixed-left')
				if (fixedright) {
			...
			let tbodys = this.$el.querySelectorAll('.blue-table-tbody')
			for (let tbody of tbodys) {
				tbody.addEventListener(
					...
					event => {
						...
						if (tr) {
							utils.addClass(tr, 'blue-table-tr-hovered')
							let index = tr.getAttribute('trIndex')
							for (let el of this.$el.querySelectorAll(
								`.blue-table-tbody>tr[trIndex='${index}']`
							) || []) {
								utils.addClass(el, 'blue-table-tr-hovered')
							}
						}
					},
					false
				...
				tbody.addEventListener(
					...
					event => {
						for (let el of this.$el.querySelectorAll(
							'.blue-table-tr-hovered'
						) || []) {
							utils.removeClass(el, 'blue-table-tr-hovered')
						}
					},
					false
				)
	...
	methods: {
		...
		getTrCls(d, index) {
			let cls = {
				'blue-table-tr-selected': this.isChecked(d),
				// eslint-disable-next-line comma-dangle
				'blue-table-tr-select-disabled': d._disabledSelect
			}
			if (this.getTrClass) {
		...
		isChecked(d) {
			return (
				this.checks.indexOf(d) > -1 ||
				(this.selectRow && d == this.rowSelected)
			)
		},
		setRowSelect(data) {
		...
		invereSelection() {
			this.checks = this.datas.filter(
				item => this.checks.indexOf(item) == -1
			)
		},
		clearSelection() {
			this.checks = []
		...
		resize() {
			this.$nextTick(() => {
				...
				this.tableWidth = this.$el.querySelector(
		...
		initFixedWidth() {
			let ths = this.$el.querySelectorAll(
				'.blue-table-header table>tr>th'
			)
			let fixedColumnLeftLength =
				this.fixedColumnLeft.length + (this.checkbox ? 1 : 0)
			let leftWidth = 0
			...
			for (
				let i = ths.length - 1;
				i > ths.length - fixedColumnRightLength - 1;
				i--
			) {
		...
		initColumns() {
			...
			if (this.$slots.default) {
				for (let slot of this.$slots.default) {
					...
					if (
						option &&
						(option.tag == 'TableItem' ||
							option.tag == 'blue-table-item' ||
							option.tag == 'blue-tableitem')
					) {
		...
		triggerTrClicked(data, index, event) {
			...
			if (
				this.checkbox &&
				this.selectWhenClickTr &&
				!utils.hasClass(event.target, 'blue-checkbox-native') &&
				!data._disabledSelect
			) {
	...
	computed: {
		isTemplateMode() {
			...
			return (
				defaultSlot &&
				(defaultSlot.name == 'normalized' || !this.$slots.default)
			)
		},
		totalCol() {
