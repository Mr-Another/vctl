<template>
  <div :class="tableCls">
    ...
    <div class="blue-table-container">
      <div class="blue-table-content-empty" v-if="datas.length == 0">
      ...
      <div
        ...
        class="blue-table-fixed-right"
        :style="fixedRightBodyStyle"
      >
        <table :style="{'margin-top': (-scrollTop+'px'), width: (tableWidth + 'px')}">
          <colgroup>
            <col
      ...
      <table :style="{width: leftWidth + 'px'}">
        ...
        <tr>
          <th v-if="checkbox" class="blue-table-th-checkbox">
            <Checkbox
              :indeterminate="checks.length>0&&checks.length<datas.length"
              :checked="datas.length > 0 && checks.length == datas.length"
              @click.native="checkAll"
            ></Checkbox>
          </th>
          <TableTh
    ...
    >
      <table :style="{width: rightWidth + 'px'}">
        <colgroup>
          <col
            v-for="(c, index) of fixedColumnRight"
            :width="getWidth(c)"
            :key="index+update.columns"
          >
        </colgroup>
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      if (body) {
        ...
        let fixedright = this.$el.querySelector(".blue-table-fixed-right");
  ...
  methods: {
    ...
    isChecked(d) {
      return (
        this.checks.indexOf(d) > -1 || (this.selectRow && d == this.rowSelected)
      );
    },
    setRowSelect(data) {
    ...
    initFixedWidth() {
      let ths = this.$el.querySelectorAll(".blue-table-header table>tr>th");
