<template>
  <div :class="pageCls">
    ...
    <Select
      ...
      v-if="orders.sizes!=-1"
    ></Select>
    <span class="blue-page-pager-container" :style="{order:orders.pager}" v-if="orders.pager!=-1 && this.count>0">
      <span :class="prevCls" @click="prev()">
        <i class="icon-left"></i>
      </span>
      <span @click="change(1)" :class="genPagerCls(1)">1</span>
      <span v-if="pagers.length > 0 && 1 < pagers[0] - 1" class="blue-page-pager blue-page-ellipsis">...</span>
      <span v-for="pager of pagers" :key="pager" @click="change(pager)" :class="genPagerCls(pager)">{{pager}}</span>
      <span class="blue-page-pager blue-page-ellipsis" v-if="pagers.length > 0 && count > pagers[pagers.lengtblue-1] + 1">...</span>
      <span @click="change(count)" :class="genPagerCls(count)" v-if="this.count>1">{{count}}</span>
      <span :class="nextCls" @click="next()">
        <i class="icon-right"></i>
      </span>
    </span>
    <input type="text" :style="{order:orders.jumper}" v-if="orders.jumper!=-1 && count > 0" class="blue-page-jumper-input blue-input" :value="curNow" @blur="jump" @keyup.enter="jump">
  </div>
</template>
<script>
