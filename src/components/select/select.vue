  ...
  <div :class="selectCls">
    <div :class="showCls">
      <template v-if="multiple">
        <div class="blue-select-multiple-tags">
          <span v-for="obj of objects" :key="obj[keyName]">
            <span>{{obj[titleName]}}</span>
            <i
              class="blue-icon-close blue-icon-down icon-close"
              @click.stop="setvalue(obj)"
              v-if="!disabled"
            ></i>
          </span>
          <input
            v-if="filterable"
            type="text"
            class="blue-select-search-input blue-input"
            v-model="searchInput"
            :disabled="disabled"
            @keyup="handle"
            @blur="blurHandle"
        ...
        <div v-if="!hasValue&&!filterable" class="blue-select-placeholder">{{showPlaceholder}}</div>
      </template>
      <template v-else>
        <template v-if="filterable">
          <input
            ...
            :disabled="disabled"
            @keypress.enter="enterHandle"
            :class="{'blue-select-search-input-value': hasValue}"
            class="blue-select-search-input blue-select-single-search-input blue-input"
            v-model="searchInput"
            :placeholder="hasValue?'':showPlaceholder"
          >
          <div
            class="blue-select-filterable-value"
            @click="focusSearchInput"
            v-if="hasValue&&searchInput===''"
          >{{singleValue}}</div>
        </template>
        <template v-else>
          <div class="blue-select-value-single" v-if="hasValue">
            <template v-if="hasValue">
              <div v-if="!$scopedSlots.show" class="blue-select-value-single">{{singleValue}}</div>
              <slot v-else :value="objects" name="show"></slot>
            </template>
          </div>
          <div v-else class="blue-select-placeholder">{{showPlaceholder}}</div>
        </template>
      </template>
      <i class="blue-icon-down icon-down"></i>
    </div>
    <div :class="groupCls">
      <div class="blue-select-group-container" v-if="isShow">
        <div class="blue-select-list">
          <slot name="top" :results="filterOptions"></slot>
          <ul class="blue-select-ul">
            <template v-for="(option, index) of filterOptions">
              ...
              >
                ...
                <i v-if="multiple" class="blue-icon-check icon-check"></i>
              </li>
            </template>
            <li v-if="filterOptions.length==0" class="blue-select-ul-empty">{{showEmptyContent}}</li>
          </ul>
...
export default {
    ...
    nowSelected() {
      this.$nextTick(() => {
        if (this.content && this.nowSelected > -1) {
          let dom = this.content.querySelector(".blue-select-item-picked");
          let uldom = this.content.querySelector(".blue-select-list");
          if (dom && uldom) {
            if (
  ...
  mounted() {
    this.$nextTick(() => {
      let el = (this.el = this.$el.querySelector(".blue-select-show"));
      let content = (this.content = this.$el.querySelector(
        ".blue-select-group"
      ));
      let that = this;
  ...
  methods: {
    focusSearchInput() {
      this.$el.querySelector(".blue-select-search-input").focus();
    },
    handle(event) {
    ...
    parse() {
      if (this.multiple) {
        ...
        if (!utils.isArray(values)) {
          console.warn(
            `[BlueUI WARNING] Select Component: value '${values}' can't be a value of a multiple select`
          );
          values = [];
        }
        this.codes = values
  ...
  computed: {
    ...
    filterOptions() {
      if (this.searchInput) {
        ...
        return this.options.filter(item => {
          return (
            (item[this.html] || item[this.titleName])
              .toLocaleLowerCase()
    ...
    options() {
      if (!this.datas && !this.dict) {
        console.error(
          "[BlueUI Error] Select Component: Datas or dict parameters need to be defined at least."
        );
        return [];
      }
