<template>
  <div :class="cascaderCls" :disabled="disabled">
    <div class="blue-cascader-show">
      <div v-if="multiple&&objects.length" class="blue-cascader-multiple-tags">
        <span v-for="(obj, index) of objects" :key="index+''+obj.key">
          <span>{{obj.title}}</span>
          <i class="icon-close-min" @click.stop="remove(obj)" v-if="!disabled"></i>
        </span>
      </div>
      <div v-else-if="!multiple&&object" class="blue-cascader-value-single">
        <span>{{object.title}}</span>
        <i class="icon-close text-hover" v-if="object&&!disabled" @mousedown="clear"></i>
      </div>
      <div v-else class="blue-cascader-placeholder">{{showPlaceholder}}</div>
    </div>
    <div :class="groupCls">
...
import CascaderItem from "@/src/components/cascader/cascader-item";

const prefix = "blue-cascader";
  ...
  computed: {
    ...
    param() {
      if (this.config) {
        return utils.extend(
          {},
          config.getOption("cascader.default"),
          config.getOption(`cascader.configs.${this.config}`),
          this.option
        );
      } else {
        return utils.extend(
