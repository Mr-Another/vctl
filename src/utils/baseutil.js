...
const baseutil = {
  ...
  clearCookie(domain, path) {
    ...
    if (keys) {
      for (let i = keys.length; i--; ) {
        let cookieString = `${keys[i]}=0;expires=${new Date(
          0
  ...
  removeCookie(name, domain, path) {
    ...
    if (name && cookieEnabled) {
      path = path || "/";
      let cookieString = `${name}=0;expires=${new Date(
        0
      ).toUTCString()};path=${path};`;
      if (domain) {
        cookieString += `domain=${domain};`;
  ...
  getAuthor() {
    const author =
      this.getURLParam("author", window.location.search) ||
      this.getLocal("window_author");
    if (author) {
...
baseutil.valueForKeypath = baseutil.getKeyValue;
baseutil.setValueForKeypath = baseutil.setKeyValue;
