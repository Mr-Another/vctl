import utils from "@/src/utils/baseutil";
...
class Validator {
  constructor(rules) {
    if (!utils.isObject(rules)) {
      console.error(
        "[HeyUI Error] Validator: Please provide the correct validation parameters"
      );
    }
  ...
  validData(data, { next, prop = "", sourceData, uuid } = {}) {
    ...
    if (utils.isArray(data)) {
      for (let i = 0; i < data.length; i++) {
        ...
        result = extendResult(
          result,
          this.validData(data[i], { next, prop: nowProp, sourceData, uuid })
        );
      }
      ...
      for (let d in data) {
        ...
        result = extendResult(
          result,
          this.validData(data[d], { next, prop: nowProp, sourceData, uuid })
        );
      }
    }
    return result;
  ...
  validField(prop, data, { next, uuid } = {}) {
    ...
    if (prop.indexOf("[") > -1) {
      ...
      combineRules = utils.extend(
        [],
        combineRules,
        this.combineRules[arrayRuleKey]
      );
    }
    ...
    if (
      ...
      utils.isFunction(rule.validAsync)
    ) {
  ...
  combineRulesValid(rules, value, parent, parentProp, uuid) {
    ...
    for (let rule of rules) {
      let result = null;
      let prop =
        (rule.parentRef && parentProp ? parentProp + "." : "") +
        rule.refs[rule.refs.length - 1];
      ...
      if (uuid && combineRuleResult.uuid == uuid + parentProp) {
        ...
        for (let ref of rule.refs) {
          ...
          let refProp =
            (rule.parentRef && parentProp ? parentProp + "." : "") + ref;
          ...
          if (
            ...
            console.log(
              "basic combine validation does not pass",
              refProp,
              this.rules[refProp],
              v
            );
