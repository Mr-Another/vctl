<template>
  <div class="blue-image-preview" :style="previewStyle">
    <span class="blue-image-preview-index">{{showIndex + 1}} / {{datas.length}}</span>
    <span
      v-if="showIndex != 0"
      class="blue-image-preview-icon blue-image-preview-left-icon"
      @click="change(showIndex - 1)"
    >
      <i class="icon-left"></i>
    </span>
    <transition name="fade">
      <img
        ...
        ref="img"
        @load="initStyle"
        class="blue-image-preview-image"
        :alt="previewFile.name"
      >
    </transition>
    <slot :data="previewFile" name="item" :index="index"></slot>
    <span
      v-if="showIndex != datas.length - 1"
      class="blue-image-preview-icon blue-image-preview-right-icon"
      @click="change(showIndex + 1)"
    >
