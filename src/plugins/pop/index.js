import Popper from "@/src/plugins/popper";
...
class Pop {
    ...
    const triggerEvents =
      typeof options.trigger === "string"
        ? options.trigger.split(" ").filter(trigger => {
            return (
              ["click", "hover", "focus", "contextMenu"].indexOf(trigger) !== -1
            );
          })
        : [];
  ...
  show(event) {
    ...
    utils.addClass(this.reference, "blue-pop-trigger");
  ...
  hide() {
    ...
    this.hideTimeout = setTimeout(() => {
      utils.removeClass(this.reference, "blue-pop-trigger");
    ...
    directtriggerEvents.forEach(event => {
      const func = evt => {
        if (evt.type == "contextmenu") {
          ...
          this.options.offset = `${evt.clientX - rect.x}, -${rect.bottom -
            evt.clientY -
            10}`;
          if (this.popperInstance) {
    ...
    if (options.closeOnClickBody) {
      this.documentHandler = e => {
        ...
        if (
          ...
          this.popNode.contains(e.target)
        ) {
          return false;
        }
        ...
        if (target.tagName != "BODY") {
          let targetTrigger = document.body.querySelector(
            `[aria-describe="${target.getAttribute("aria-describedby")}"]`
          );
  ...
  setPopNodeEvent() {
    ...
    this.popNode.addEventListener("mouseout", event => {
      ...
      if (
        !this.popNode.contains(relatedreference) &&
        relatedreference != this.reference &&
        !this.reference.contains(relatedreference)
      ) {
        this.hide();
      }
    });
